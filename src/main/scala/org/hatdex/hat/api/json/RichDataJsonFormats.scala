...
trait RichDataJsonFormats extends HatJsonFormats with JodaWrites with JodaReads {
  ...
  val endpointDataWrites: Writes[EndpointData] = ((__ \ "endpoint").write[String] and
  ...
  implicit val apiFieldTransformationFormat: Format[FieldTransformation.Transformation] =
    new Format[FieldTransformation.Transformation] {
      def reads(json: JsValue): JsResult[FieldTransformation.Transformation] =
        (json \ "transformation").as[String] match {
          case "identity" => JsSuccess(FieldTransformation.Identity())
          case "datetimeExtract" =>
            Json.fromJson[FieldTransformation.DateTimeExtract](json)(fieldTransDateTimeExtractFormat)
          case "timestampExtract" =>
            Json.fromJson[FieldTransformation.TimestampExtract](json)(fieldTransTimestampExtractFormat)
      ...
      def writes(transformation: FieldTransformation.Transformation): JsValue = {
        val (transformed, tType) = transformation match {
          ...
          case ds: FieldTransformation.DateTimeExtract =>
            (Json.toJson(ds)(fieldTransDateTimeExtractFormat), JsString("datetimeExtract"))
          case ds: FieldTransformation.TimestampExtract =>
            (Json.toJson(ds)(fieldTransTimestampExtractFormat), JsString("timestampExtract"))
  ...
  implicit val apiFilterOperatorFormat: Format[FilterOperator.Operator] = new Format[FilterOperator.Operator] {
    def reads(json: JsValue): JsResult[FilterOperator.Operator] =
      (json \ "operator").as[String] match {
  ...
  val endpointQueryWrites: Writes[EndpointQuery] = ((__ \ "endpoint").write[String] and
  ...
  implicit val endpointDatabundleFormat: Format[EndpointDataBundle] =
    Format(endpointDatabundleRead, endpointDatabundleWrite)
  ...
  private val richDataDebitReads: Reads[RichDataDebit] = ((__ \ "dataDebitKey")
    .read[String]
    .filter(JsonValidationError("Data Debit Key invalid"))(_.matches("[0-9a-zA-Z-]+")) and
  ...
  implicit val durationReads: Reads[Duration] = new Reads[Duration] {
    def reads(json: JsValue): JsResult[Duration] =
      json match {
        ...
        case JsString(value) =>
          Try(Duration.parse(value))
            .map(p => JsSuccess(p))
            .recover({
              case e => JsError(Seq(JsPath() -> Seq(JsonValidationError(s"Could not parse period: ${e.getMessage}"))))
            })
            .get
  ...
  private val dataDebitPermissionsWrites: Writes[DataDebitPermissions] = Json
    .writes[DataDebitPermissions]
    .addField("active", _.active)
    .addField("end", _.end)

  private val dataDebitPermissionsReads: Reads[DataDebitPermissions] = Json.reads[DataDebitPermissions]
  implicit val dataDebitPermissionsFormat: Format[DataDebitPermissions] =
    Format(dataDebitPermissionsReads, dataDebitPermissionsWrites)

  private val ddWrites: Writes[DataDebit] = Json
    .writes[DataDebit]
    .addField("active", _.active)
